import { fail, hasValue, isArray, isObject, isString, noValue, splitList } from '@abw/badger-utils'
import { parseAttrs } from './attrs.js'
import { joinClasses } from './classes.js'
import { DASH, DEFAULT_ICON_WIDTH, DEFAULT_ICON_HEIGHT, FILL, PATH, POLYLINE, POLYGON, SVG, ARRAY, CIRCLE, ELLIPSE, LINE, RECT } from '../constants.js'
...
export const resolveIconData = (uri, library, props={}) => {
  const { icons } = library
  // const { style: propStyle, transfor: propTransform, ...otherProps } = props
...
export const expandIconData = (data, library, props={}) => {
  ...
  if (isObject(data)) {
    return expandIconObject(icon, data)
  }
...
export function expandIconString(icon, string) {
  ...
  if (match) {
    ...
    console.log(`matched [${element}] ${data}]`)
    ...
    if (classes.length) {
      ...
      console.log(`adding classes: ${icon.className}`)
    }
...
export function expandIconObject(icon, object) {
  const body = { ...object }
  if (body.element) {
